...
def tokenize_mistral(batch, tokenizer, device, test=False):
    prompt, label = batch["text"], batch["labels"]
    mask_token = -100  # ignore_index of CrossEntropyLoss
    if test or not label:
        # tokenizer.padding_side = 'left'
        tokens = tokenizer(list(prompt), return_tensors="pt", truncation=True,max_length=512,padding="max_length",)
        tokens["labels"] = tokenizer(list(label), return_tensors="pt", truncation=True,max_length=512,padding="max_length",)["input_ids"]
        ...
        prompt_ids = tokenizer(list(prompt), return_tensors="pt", truncation=True,max_length=512,padding="max_length",)["input_ids"]
        num_prompt_toks = [int((i != tokenizer.pad_token_id).sum()) for i in prompt_ids]
        tokens = tokenizer(full_prompt, return_tensors="pt", truncation=True,max_length=512,padding="max_length",)
